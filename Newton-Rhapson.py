...
error = input("Error: ") #Establecemos el numero de decimales cuyo error buscamos
error = int(error)
...
for n in ceros_flotantes:
    ...
    comp2 = round(n - expr.subs(x,n)/expr.diff(x).subs(x,n),error) #Y para comparar la primera iteración
    num = n # X0
    while (comp1 != comp2 and contador<Max_iter): #Mientras X != f(x) con el error que tenemos
        contador += 1
        num = num - expr.subs(x,num)/expr.diff(x).subs(x,num)   # Xn+1 = Xn + f(Xn)/f'(Xn)
        comp1 = comp2 #Sustituimos los valores que vamos a comparar para seguir iterando
        comp2 = round(num - expr.subs(x,num)/sp.diff(expr,x).subs(x,num),error)
        print(comp2) 